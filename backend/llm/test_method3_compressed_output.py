# ============================================================
# 방법 3: 결과값 글자 수 제한 + JSON 압축 + medium effort
# - 출력 JSON 압축 (설명 최소화)
# - 글자 수 제한 명시
...
※ 글자 수 제한 (반드시 준수):
- title: 최대 30자
- description: 최대 100자 (핵심만 간결하게)
- attackPath: 각 단계 최대 30자, 최대 3단계
- impact: 최대 50자
- recommendation: 최대 80자

※ JSON 압축: 불필요한 공백 없이 compact하게 출력

스키마
{{"summary":{{"high":0,"medium":0,"low":0}},"vulnerabilities":[{{"severity":"high|medium|low","title":"제목(30자)","description":"설명(100자)","attackPath":["단계1","단계2"],"impact":"영향(50자)","recommendation":"권장(80자)","cvss_score":0.0}}]}}
"""
    ...
    payload = {
        "messages": [
            {
                "role": "system",
                "content": "너는 전 세계 기업 환경을 대상으로 실전 침투 시나리오를 설계하고 검증하는 Tier-1 클라우드 보안 아키텍트이자 레드팀 리더이다. 출력은 반드시 글자 수 제한을 준수하고 압축된 JSON으로 출력하라."
            },
            {
                "role": "user",
        ...
        "temperature": 0.2,
        "top_p": 0.9,
        "reasoning_effort": "medium"  # 방법 3: medium
    }
    ...
    try:
        ...
        print("\n" + "="*60)
        print("📊 [방법 3] Compressed Output + Medium Effort 결과")
        print("="*60)
...
if __name__ == "__main__":
    print("\n🚀 [방법 3] Compressed Output + Medium Effort 테스트 시작...")
    print("설정: 글자 수 제한 + JSON 압축 + reasoning_effort='medium'")
