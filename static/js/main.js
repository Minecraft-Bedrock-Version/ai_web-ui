        ...
        function updateNavigation() {
            ...
            // 취약점 분석 완료 전까지 다음 버튼 비활성화
            if (currentStep === 2 && !analysisCompleted) {
    nextBtn.disabled = true;
        ...
        function resetForm() {
            ...
            document.getElementById('analysisProgress').classList.add('hidden'); // 🔥 핵심
document.getElementById('analysisResults').classList.add('hidden');
document.getElementById('analysisProgressBar').style.width = '0%';
document.getElementById('progressText').textContent = '0% 완료';
...
function runAnalysis() {
    ...
    // 🔥 버튼 영역 자체 숨김
    document.getElementById('analysisButtonWrapper').style.display = 'none';
            ...
            vulnerabilities.forEach(vuln => {
                ...
                card.innerHTML = `
                    <div style="display: flex; align-items: start; justify-content: space-between; margin-bottom: 12px;">
                        <h3 style="font-weight: 600; margin-bottom: 8px;">${vuln.title}</h3>
                        <span class="badge ${badgeClass}">${vuln.severity.toUpperCase()}</span>
                    </div>
                    <p style="color: #374151; margin-bottom: 12px; font-size: 14px;">${vuln.description}</p>
                    <p style="font-size: 13px; font-weight: 500; color: #374151; margin-bottom: 8px;">공격 경로 (Attack Path):</p>
                    ${attackPathHTML}
