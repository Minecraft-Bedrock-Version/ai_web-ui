...
async function submitCreateResource() {
  ...
  const payload = {
    state: {
      ...
      selectedEntity: name,             // 생성할 리소스 이름
      activePolicies: managedPolicies, // 선택한 정책 리스트
    },
    region: state.region
  };
...
async function submitAttachManagedPolicies() {
  ...
  const payload = {
    state: {
      ...
      selectedEntity: state.selectedEntity,    // 현재 선택된 리소스 이름 (예: admin-user)
      activePolicies: selectedArns,        // 선택된 ARN 리스트 배열
    },
    region: state.region
  };
